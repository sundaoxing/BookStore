...
<c:forEach items="${orderList}" var="order">
		<tr bgcolor="gray" bordercolor="gray">
			...
			<span style="color: red">买家未付款</span>
			</c:when>
			<c:when test="${order.state eq 2}">
			<a href="<c:url value='/admin/AdminOrderServlet?method=delivery&oid=${order.oid}'/>">发货</a>
			</c:when>
			<c:when test="${order.state eq 3}"><span style="color: red">等待买家确认收货</span></c:when>
